...
function normalizeShopGroups(data) {
  ...
  (data.shops || []).forEach((item) => {
    ...
    if (!grouped[key])
      grouped[key] = { key: key, label: key || "資訊", items: [] };
    grouped[key].items.push(item);
  });
  return Object.values(grouped);
}
...
function extraFields(obj, hiddenKeys) {
  return Object.entries(obj || {}).filter(
    ([k, v]) => !hiddenKeys.includes(k) && nonEmpty(v),
  );
}
...
function renderExtraRows(entries) {
  ...
  return (
    ...
    entries
      .map(
        ([k, v]) =>
          "<div class='extra-row'><strong>" +
          escapeHtml(k) +
          "：</strong>" +
          escapeHtml(Array.isArray(v) ? v.join("、") : v) +
          "</div>",
      )
      .join("") +
    "</div>"
...
function buildMapButton(mapId) {
  ...
  return (
    "<button type='button' class='btn map-switch-btn' data-mapid='" +
    escapeHtml(mapId) +
    "'>切換地圖</button>"
  );
}
...
function focusMapById(mapId) {
  ...
  document.getElementById("mapFocus").innerHTML =
    ...
    (nonEmpty(item.address)
      ? '<div style="margin-top:10px"><strong>地址：</strong>' +
    ...
    escapeHtml(item.map) +
    "' target='_blank' rel='noopener noreferrer'>開啟 Google Maps</a>" +
    "</div>";
  updateActiveStates();
}
...
function rebuildLocationList(locations) {
  ...
  if (!locations.length) {
    ...
    document.getElementById("mapFocus").innerHTML =
      '<div class="muted">目前沒有可顯示的 map / address 資料。</div>';
    return;
  }
  locations.forEach((loc, idx) => {
    ...
    div.innerHTML =
      ...
      "</div>" +
      (nonEmpty(loc.address)
        ? '<div class="loc-sub">' + escapeHtml(loc.address) + "</div>"
        : "");
  ...
  shopGroups.forEach((group, groupIndex) =>
    (group.items || []).forEach((item, itemIndex) => {
      ...
      locations.push({
        ...
        map,
        source:
          (data.shop_tabs_name || "資訊分類") + " / " + (group.label || "資訊"),
        title: item.name || "店家",
...
function render(data) {
  ...
  document.getElementById("budgetPerPerson").textContent =
    money(data.budget_per_person || 0) + " / 人";
  document.getElementById("nights").textContent = data.nights || "";
  document.getElementById("dayTabsName").textContent =
    data.day_tabs_name || "每日行程";
  document.getElementById("stayTabsName").textContent =
    data.stay_tabs_name || "住宿資訊卡";
  document.getElementById("shopTabsName").textContent =
    data.shop_tabs_name || "店家 / 活動 / 交通資訊卡";
  document.getElementById("budgetTabsName").textContent =
    data.budget_tabs_name || "預算分類";
  ...
  function renderDay(key) {
    const day =
      (data.days || []).find((d) => d.key === key) || (data.days || [])[0];
    if (!day) {
    ...
    (day.stops || []).forEach((stop, idx) => {
      ...
      stopsHtml +=
        ...
        "'" +
        (mapTarget ? " data-mapid='" + escapeHtml(mapId) + "'" : "") +
        ">" +
        "<div class='stop-top'><div><div class='stop-title'>" +
        escapeHtml(stop.name || "") +
        (stop.highlight ? " <span class='badge'>重點</span>" : "") +
        "</div>" +
        "<div class='stop-meta'>" +
        (nonEmpty(stop.time)
          ? "<span class='time-pill'>" + escapeHtml(stop.time) + "</span>"
          : "") +
        (nonEmpty(stop.stay)
          ? "<span class='pill'>" + escapeHtml(stop.stay) + "</span>"
          : "") +
        (nonEmpty(stop.type)
          ? "<span class='pill'>" + escapeHtml(stop.type) + "</span>"
          : "") +
        "</div></div>" +
        (nonEmpty(stop.cost)
          ? "<div class='cost-pill'>" + escapeHtml(stop.cost) + "</div>"
          : "") +
        "</div>" +
        "<div class='addr-box'><div class='box'><div class='small muted'>地址</div><div>" +
        ...
        (nonEmpty(stop.next)
          ? "<span class='pill'>前往下一站：" +
          escapeHtml(stop.next) +
          "</span>"
          : "") +
        "</div>" +
        "</div>";
    ...
    dayContent.innerHTML =
      "<div class='day-header'><div class='small muted'>" +
      escapeHtml(day.label || "") +
      "</div><div style='font-size:26px;font-weight:800;margin-top:4px'>" +
      ...
      (nonEmpty(day.theme)
        ...
        : "") +
      (nonEmpty(day.hero)
        ? "<div style='margin-top:10px'><span class='badge'>重點：" +
        escapeHtml(day.hero) +
        "</span></div>"
        : "") +
      "</div><div class='stops'>" +
    ...
    stayContent.innerHTML =
      ...
      (group.items || [])
        .map((s, idx) => {
          ...
          const extras = renderExtraRows(
            extraFields(s, ["area", "name", "note", "link", "map", "address"]),
          );
          return (
            "<div class='item-card" +
            (mapTarget ? " map-target" : "") +
            ...
            (nonEmpty(s.note)
              ...
              : "") +
            (nonEmpty(s.address)
              ? "<div class='extra-row' style='margin-top:12px'><strong>地址：</strong>" +
            ...
            (nonEmpty(s.link)
              ? "<a class='btn secondary' href='" +
              escapeHtml(s.link) +
              "' target='_blank' rel='noopener noreferrer'>查看住宿</a>"
              : "") +
            "</div></div>"
          );
        })
  ...
  function renderShopGroup(key) {
    ...
    if (!group) {
      shopContent.innerHTML =
        '<div class="item-card">沒有店家 / 活動資料</div>';
      return;
    }
    shopContent.innerHTML =
      "<div class='list'>" +
      (group.items || [])
        .map((s, idx) => {
          ...
          return (
            ...
            escapeHtml(s.name || "") +
            "</strong><span class='badge'>" +
            escapeHtml(s.tag || group.label || "資訊") +
            "</span></div>" +
            (nonEmpty(s.price)
              ? "<div style='margin-top:8px'>" + escapeHtml(s.price) + "</div>"
              : "") +
            (nonEmpty(s.note)
            ...
            (nonEmpty(s.link)
              ? "<a class='btn secondary' href='" +
              escapeHtml(s.link) +
              "' target='_blank' rel='noopener noreferrer'>查看連結</a>"
              : "") +
            "</div></div>"
          );
        })
  ...
  const grandTotal = (data.budget_items || []).reduce(
  ...
  function renderBudget(label) {
    if (label === "總額") {
      budgetContent.innerHTML =
        "<div class='item-card'><div style='display:flex;justify-content:space-between;gap:10px'><strong>總額</strong><span class='badge'>" +
        money(grandTotal) +
        "</span></div><div class='details-wrap'>" +
        (data.budget_items || [])
          .map(
            (item) =>
              "<div class='details-row'><div><strong>" +
          ...
          .join("") +
        "</div><div class='summary-box'><div class='small' style='opacity:.8'>全部分類加總</div><div style='font-size:24px;font-weight:800;margin-top:6px'>" +
        money(grandTotal) +
        "</div></div></div>";
    } else {
      const item =
        (data.budget_items || []).find((b) => b.label === label) ||
        (data.budget_items || [])[0];
      if (!item) return;
      const details = Array.isArray(item.details) ? item.details : [];
      const detailsSum = details.reduce((a, b) => a + Number(b.amount || 0), 0);
      const pct = grandTotal
        ? Math.round((Number(item.value || 0) / grandTotal) * 100)
        : 0;
      budgetContent.innerHTML =
        "<div class='item-card'><div style='display:flex;justify-content:space-between;gap:10px'><strong>" +
        escapeHtml(item.label || "") +
        "</strong><span class='badge'>" +
        money(item.value || 0) +
        "</span></div><div style='margin-top:10px' class='budget-bar'><div style='width:" +
        pct +
        "%'></div></div><div style='margin-top:12px' class='small muted'>分類占比：" +
        pct +
        "%</div>" +
        (details.length
          ...
          details
            ...
            .join("") +
          "</div><div class='summary-box'><div class='small' style='opacity:.8'>明細加總</div><div style='font-size:24px;font-weight:800;margin-top:6px'>" +
          money(detailsSum) +
          "</div></div>"
          : "<div class='small muted' style='margin-top:12px'>尚未提供明細</div>") +
        "</div>";
    }
...
async function loadJsonFile(file) {
  ...
  try {
    ...
    if (!validation.valid) {
      console.error("Uploaded JSON validation failed:", validation.errors);
      setStatus('JSON 格式錯誤');
...
function validateTripData(data) {
  ...
  if (Array.isArray(data.days)) {
    data.days.forEach((day, dayIndex) => {
      ...
      if (!isNonEmptyString(day.key)) {
        errors.push(`days[${dayIndex}].key 必填且必須是字串`);
      }

      if (day.label !== undefined && typeof day.label !== "string") {
        errors.push(`days[${dayIndex}].label 必須是字串`);
      }

      if (day.title !== undefined && typeof day.title !== "string") {
        errors.push(`days[${dayIndex}].title 必須是字串`);
      }
      ...
      if (Array.isArray(day.stops)) {
        day.stops.forEach((stop, stopIndex) => {
          if (!isObject(stop)) {
            errors.push(`days[${dayIndex}].stops[${stopIndex}] 必須是 object`);
            return;
          }

          if (!isNonEmptyString(stop.name)) {
            errors.push(`days[${dayIndex}].stops[${stopIndex}].name 必填且必須是字串`);
          }

          if (stop.time !== undefined && typeof stop.time !== "string") {
            errors.push(`days[${dayIndex}].stops[${stopIndex}].time 必須是字串`);
          }

          if (stop.address !== undefined && typeof stop.address !== "string") {
            errors.push(`days[${dayIndex}].stops[${stopIndex}].address 必須是字串`);
          }

          if (stop.map !== undefined && typeof stop.map !== "string") {
            errors.push(`days[${dayIndex}].stops[${stopIndex}].map 必須是字串`);
          }
        });
      }
    });
  ...
  if (data.budget_items !== undefined && !Array.isArray(data.budget_items)) {
  ...
  if (Array.isArray(data.budget_items)) {
    data.budget_items.forEach((item, index) => {
      if (!isObject(item)) {
        errors.push(`budget_items[${index}] 必須是 object`);
        return;
      }

      if (!isNonEmptyString(item.label)) {
        errors.push(`budget_items[${index}].label 必填且必須是字串`);
      }

      if (typeof item.value !== "number") {
        errors.push(`budget_items[${index}].value 必填且必須是數字`);
      }

      if (item.details !== undefined && !Array.isArray(item.details)) {
        errors.push(`budget_items[${index}].details 必須是陣列`);
      }
    });
  }
...
async function loadDefaultJson() {
  try {
    ...
    if (!res.ok) throw new Error("Failed to load trip.json: " + res.status);

    const parsed = await res.json();
    const validation = validateTripData(parsed);

    if (!validation.valid) {
      console.error("Trip schema validation failed:", validation.errors);
      setStatus("trip.json 格式錯誤");
      renderValidationError(validation.errors);
      return;
    }
    ...
    const trip = getTrip();
    setStatus(trip ? `目前行程：${trip}` : "目前使用預設 trip.json");
  } catch (err) {
    console.error(err);
    const isFileProtocol = window.location.protocol === "file:";
    setStatus(
      isFileProtocol
        ? "請用 GitHub Pages 或本機 HTTP server 開啟，file:// 無法讀取 JSON"
        : "trip.json 載入失敗"
    );
  }
}
...
dropzone.addEventListener("drop", (e) => {
  const file = [...(e.dataTransfer?.files || [])].find(
    (f) => /\.json$/i.test(f.name) || (f.type && f.type.includes("json")),
  );
  loadJsonFile(file);
});
